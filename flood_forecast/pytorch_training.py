...
def compute_validation(validation_loader: DataLoader,
    ...
    if classification:
        ...
        label_list = label_list[:, 0, :].cpu()
        mod_output1 = torch.cat(mod_output_list)[:, 0, :].cpu()
        fin = label_list.max(dim=1)[1].cpu()
        ...
        wandb.log({"conf_": wandb.plot.confusion_matrix(probs=mod_output1.detach().numpy(),
                   y_true=fin.detach().numpy(), class_names=None)})
    model.train()
