...
def stream_baseline(river_flow_df: pd.DataFrame, forecast_column: str,
                    hours_forecast=336) -> (pd.DataFrame, float):
    """
    ...
    mse_baseline = sklearn.metrics.mean_squared_error(
        test_river_data[forecast_column],
        test_river_data["predicted_baseline"])
    print(mse_baseline)
...
def evaluate_model(
    ...
    for evaluation_metric in evaluation_metrics:
        for target in target_col:
            ...
            s = evaluation_metric_function(torch.from_numpy(
                df_train_and_test[target][forecast_history:].to_numpy()), end_tensor)
...
def infer_on_torch_model(
    ...
    model.model.eval()
    history, df_train_and_test, forecast_start_idx = csv_test_loader.get_from_start_date(
        datetime_start)
    ...
    if num_prediction_samples is not None:
        ...
        df_prediction_samples = pd.DataFrame(
            index=df_train_and_test.index, columns=list(
                range(num_prediction_samples)), dtype='float')
...
def generate_predictions(
        ...
        decoder_params: Dict) -> torch.Tensor:
    ...
    if decoder_params is None:
        ...
        # real_target:torch.Tensor, start_symbol:torch.Tensor, unsqueeze_dim=1,
        # device='cpu')
...
def generate_predictions_non_decoded(
        ...
        forecast_length: int,
        hours_to_forecast: int) -> torch.Tensor:
    ...
    for i in range(0, int(np.ceil(hours_to_forecast / forecast_length).item())):
        ...
        if test_data.use_real_precip and test_data.use_real_temp:
            ...
            # Hmm right now this will create a bug if for some reason the order
            # [precip, temp, output]
...
def generate_decoded_predictions(
    ...
    real_target_tensor = torch.from_numpy(test_data.df[forecast_start_idx:].to_numpy()).to(
        device).unsqueeze(0).to(model.device)
...
def generate_prediction_samples(
        ...
        hours_to_forecast: int,
        decoder_params: Dict,
        num_prediction_samples: int) -> np.ndarray:
    pred_samples = []
