...
class PlotFunctionsTest(unittest.TestCase):
    df_test = pd.DataFrame({
        ...
        'target_col': [4.0, 5.0, 6.0, 7.0]
    })
    df_preds = pd.DataFrame({
        0: [-1.0, -2.0, -1.0, 0.0],
        1: [1.0, 2.0, 4.0, 3.0]
    })
    df_preds_empty = pd.DataFrame(index=[0, 1, 2, 3])

    def test_calculate_confidence_intervals(self):
        ci_lower, ci_upper = 0.025, 0.975
        df_quantiles = calculate_confidence_intervals(self.df_preds, ci_lower, ci_upper)
    ...
    def test_calculate_confidence_intervals_df_preds_empty(self):
        ci_lower, ci_upper = 0.025, 0.975
        df_quantiles = calculate_confidence_intervals(self.df_preds_empty, ci_lower, ci_upper)
